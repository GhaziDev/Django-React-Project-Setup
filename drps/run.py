...
def start_project():
    ...
    python_command = f"cd {path} &&  mkdir {project_name} && cd {path}\\{project_name} && django-admin startproject {django_project_name} && cd {path}\\{project_name}\\{django_project_name} && python -m venv {venv_name} && django-admin startapp {django_app_name} &&  {venv_name}\\Scripts\\activate && python -m pip install django && python -m pip install django-rest-framework && python -m pip install django-cors-headers && python -m pip install django-extensions"
